...
$kg = "CRD"
$sum = "[acjmnsv][aklnsyz][cbgomspt]" + "wsr*"
###################################################################
...
$Jobs = Get-AzRecoveryServicesBackupJob -VaultId $Vault_ID.Id | where-object {$_.WorkLoadname -like "$sum"} -WarningAction SilentlyContinue
