...
### Important: Create SERVERLIST.TXT file in C:\TEMP folder and add servers to be checked

### Contstants
$log = 'C:\Temp\CHK_Everyone.txt'
$serverlist = get-content "C:\temp\serverlist.txt"

### Function Get-ChildItem2.ps1
...
foreach ($server in $serverlist)
{
    ...
    $shares = (net view $server)
    if ($shares -eq "There are no entries in the list.")
    {
        Write-Output "No Shared folders in $server" | Out-file $log -append
    }
    else
    {
        ...
        {
            $source = $server + '\' + $share
            $folders = Get-ChildItem2 $source -Directory -Recurse -Depth 3
