$folders = Get-ChildItem2 '\\deuwsr8001\data$\Pools' -Directory -Recurse
        ...
        ForEach ($acl in $acls.Access){
            If ($acl.IdentityReference -eq "BUILTIN\Administrators"){
                If ($acl.IsInherited -eq $False){ 
                    ...
                    Write-Output $folder.FullName | Out-file D:\Alb\ChangeFolderPermission\Log\LogDeuwsr8001P.txt -append
                }
            }
        }
}