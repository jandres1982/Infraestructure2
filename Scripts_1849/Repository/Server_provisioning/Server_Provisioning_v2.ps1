...
Invoke-command -computername $IP -ScriptBlock {$Global_creds = $args[0]; Add-Computer -DomainName global.schindler.com -Restart -Credential $Global_Creds} -Credential $creds -ArgumentList ($Global_Creds)
...
Invoke-Command -ComputerName $Hostname -ScriptBlock { & cmd.exe /c "msiexec.exe /i c:\temp\Source\SEP_14.2.4814.1101\Sep64.msi" /qn SYMREBOOT=ReallySuppress}
