$Source = "D:\Repository\Working\Antonio\Zabbix_Install\Install.cmd" #Script que vas a copiar en los servidores, si es un ps1 hay que modificar la ejecución del $action
$Destination = "C$\temp" #Donde vas a copiar ese script
$Servers = gc "D:\Repository\Working\Antonio\Zabbix_Install\Server_List.txt" #Listado de servidores
...
Invoke-Command -ComputerName $Server -Scriptblock { #aqui debes poner tu usuario
    ...
    $principal = New-ScheduledTaskPrincipal -RunLevel Highest #aqui se dice con que tipo de cuenta se ejecutua (BUILTIN la recomendada)
