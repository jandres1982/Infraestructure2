...
if ($VMS -ne $null){
    ...
    if ($CpuLimitOldMHz -ne $CpuLimitMHz) {
        ...
        "ResPoolName:  $ResPoolName CPU Limit changed - CPU Ratio: $Ratio Memory: $MemoryCountGB - Old Limit Mhz: $CpuLimitOldMHz New Limit Mhz:$CpuLimitMHz CPU Count: $cpucount" | Out-File -FilePath $Logfile -Append
...
#        Set-ResourcePool -ResourcePool $ResPoolName -CpuLimitMhz $CpuLimitMHz
       ...
       "ResPoolName:  $ResPoolName CPU Limit has not changed - Memory: $MemoryCountGB Limit: $CpuLimitMHz CPU Count: $cpucount" | Out-File -FilePath $Logfile -Append
    }
}
else{
...
$To = "VirtualPlatform.SPOC@swisscom.com", "scc.support@ch.schindler.com", "ServiceManagement.Schindler@swisscom.com", "Erich.Niffeler@swisscom.com"
$ResPoolStartWith = "RP_SCH-*"
$Date = Get-date -Format "yyyy-MM-d"
$LogFileName = "TestSetCPULimits-$Date.log"
...
Foreach ($Cluster in $Clusters){
    ...
    # Write Results to Log if the cluster has any Ressource Pools
    if ($Logging -eq $true){
        $UpdatedCluster = Get-Cluster -Name $Cluster.name # Get the updated Cluster information
        $ClusterTotalCpuMHz = ($UpdatedCluster | Get-View).Summary.TotalCpu
        $ClusterCpuLimitMHz = ($UpdatedCluster | Get-ResourcePool "Resources").CpuLimitMHz
        $CpuRessourceAllocation = [math]::Round($CPULimitSum / $ClusterCpuLimitMHz * 100)
        $CRV = Get-ClusterResValue -Clustername $cluster.name
        " " | Out-File -FilePath $Logfile -Append
        $Body0 = "CPU Limits Cluster $cluster" + $OFS + "Total (MHz)   : $ClusterTotalCpuMHz" + $OFS + "ClLimit (MHz) : $ClusterCpuLimitMHz" + $OFS + "PoolSum (MHz) : $CPULimitSum" + $OFS + "CPUAllocation : $CpuRessourceAllocation %" + $OFS + "CPUUsage (MHz): $($CRV.CpuUsageMhz)" + $OFS + "CPUUsage      : $($CRV.CPUUsageP) %"
        $Body0 | Out-File -FilePath $Logfile -Append
        $Body1 = $OFS + "MEM Limits Cluster $cluster" + $OFS + "Total (GB): $($CRV.MemoryTotalGB)" + $OFS + "MEMUsage (GB) : $($CRV.MemoryUsageGB)" + $OFS + "MEMUsage      : $($CRV.MemoryUsageP) %" 
        $Body1 | Out-File -FilePath $Logfile -Append
        $Body2 = $UpdatedCluster | Get-ResourcePool | Select-Object Name, CpuLimitMHz | Out-String
        $Body2 | Out-File -FilePath $Logfile -Append
        $Body3 = "==> Cluster CPU Ressource allocation is at $CpuRessourceAllocation %" 
        # Send a Mail Alert when CPU Limit has reached the Alarm Level (e.g. 70% of available Cluster Ressources) and Alert is new
        if ((($CpuRessourceAllocation -ge $AlertLevel) -or ($CRV.MemoryUsageP -ge $AlertLevel)) -and !(Test-Path $LogPath$AlertFile)){
            ...
            $Body = "$BodyText `n `n  $Body0 `n $Body1 `n $Body2 `n $Body3"
