...
$Username = "global\svcshhvcmaint"
$Path = "D:\Scripts\Schindler\Vmware\VMtools_Restart_Hanging\SHH\"
$CredsFile = "$Path$AdminName-VCCreds.txt"

$FileExists = Test-Path $CredsFile

if  ($FileExists -eq $false) {
    $Cred = Get-Credential -Message "VCenterSHH Credentials" -UserName $username
...
#Get View of all "poweredon" "Windows Server" VMs in Datacenter "SHH" that are on Tools Version "10240" and "Tools are NOT running".
$vmviews = (get-view -ViewType Virtualmachine -SearchRoot (get-datacenter SHH).id | Where-Object {$_.runtime.PowerState -like "poweredOn" -and $_.config.tools.toolsVersion -like "10240"-and $_.Guest.ToolsStatus -like "toolsNotRunning"-and $_.config.GuestFullName -like "*Windows Server*"}).name

#Filter out DMZ_Cluster and TST* VMs because we need to run with different credentials... and in different script on sdbwsr0001 (DMZ mgmt server)
$vms = (get-vm $vmviews | Where-Object {$_.VMHost.Parent -notlike "DMZ_Cluster" -and $_.name -notlike "tst*"}).name
