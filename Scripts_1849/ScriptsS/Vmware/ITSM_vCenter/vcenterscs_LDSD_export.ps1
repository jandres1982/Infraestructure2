...
$hosts = Get-View -ViewType Hostsystem | Select-Object -Property @{N="H_NAME";E={$_.Name}},
        ...
        @{N="HostLocation";E={($_.summary.customvalue | Where-Object {$_.Key -like "101"}).value}},
...
invoke-sqlcmd -query "TRUNCATE TABLE vCenterExport.dbo.LDSD_ClientInfo"  -serverinstance $ServerInstance -database $DB -Username $SQLUser -Password $SQLpw
...
INSERT INTO dbo.LDSD_ClientInfo (DNS_Name, Hostname, Guest_OS, ID, Host_ID, NUM_VCPU, IP_ADDRESS, HARDWARE_CORES, NumOfSockets, G_Powerstate, G_State, G_Mem_MB) 
...
invoke-sqlcmd -query "TRUNCATE TABLE vCenterExport.dbo.LDSD_HostVMList"  -serverinstance $ServerInstance -database $DB -Username $SQLUser -Password $SQLpw
...
INSERT INTO dbo.LDSD_HostVMList (H_NAME, Hostname, HostLocation, HOST_MODEL, H_enabled, H_CPUmodel, H_CPUcount, H_CoresSocket, H_Corecount, HOST_VENDOR, H_Host_ID, H_Mem_Size_MB, H_Powerstate, ClusterName, MAC_ADDRESS, PRODUCT_NAME, PRODUCT_VERSION, PRODUCT_BUILD, PRODUCT_OS_TYPE, NameVersion, CPU_GHz, IP_ADDRESS, Inventory_Date, prefix) 
VALUES ('$($esxhost.H_NAME)', '$($esxhost.Hostname)','$($esxhost.HostLocation)', '$($esxhost.HOST_MODEL)', '$($esxhost.H_enabled)', '$($esxhost.H_CPUmodel)', '$($esxhost.H_CPUcount)', '$($esxhost.H_CoresSocket)', '$($esxhost.H_Corecount)', '$($esxhost.HOST_VENDOR)', '$($esxhost.H_Host_ID)', '$($esxhost.H_Mem_Size_MB)', '$($esxhost.H_Powerstate)', '$($esxhost.ClusterName)', '$($esxhost.MAC_ADDRESS)', '$($esxhost.PRODUCT_NAME)', '$($esxhost.PRODUCT_VERSION)', '$($esxhost.PRODUCT_BUILD)', '$($esxhost.PRODUCT_OS_TYPE)', '$($esxhost.NameVersion)', '$($esxhost.CPU_GHz)', '$($esxhost.IP_ADDRESS)', '$($esxhost.Inventory_Date)', '$($esxhost.prefix)')
