...
$mail = "michael.barmettler@ch.schindler.com", "erich.niffeler@ch.schindler.com" 
...
#Get date / time now for reporting
...
# NIE: New creation of System list
$data = $migdata | Where-Object {($_.rhserver_lsend -ne "") -and ($_.rhserver_ls -eq "1")}
$systems = $data | Where-Object {$_.rhserver_lsend -ge "$lasthour"}
...
foreach ($system in $systems)
 ...
 $servername = $system.server_name
 $serversite = $system.server_site
 "Coreid: $coreid , Servername: $servername , Serversite: $serversite"
 #$setlocation
 if ($setlocation.ToString() -match "OK:")
 {
 #OK, Location set:
 $OK = "$time - $($system.server_name) - $setlocation - $($system.server_site)"
 Write-Host $OK
 $OK | Out-File -FilePath "D:\Scripts\Schindler\Microsoft\ITSM_SetLocation\log\log.txt" -Append
